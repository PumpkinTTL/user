          ...
          <div class="space-y-4">
            <template v-for="item in contactList" :key="item.key">
              <a v-if="item.isLink" :href="item.value" target="_blank" rel="noopener noreferrer"
                ...
                <svg class="w-5 h-5 flex-shrink-0" :style="{ color: item.color }" fill="currentColor" :viewBox="item.viewBox">
                  <path :d="item.path" />
                </svg>
...
const contactList = computed(() => getContactList(config.value?.contact as Record<string, unknown> | undefined))
